...
public class Main {
	...
	private static final int PERFECT_PARTICLES = RenderTexture.perfectCount(8192);
	...
	private void init() {
		...
		for (int i = 0; i < PERFECT_PARTICLES; i++) {
			pos[i] = new Vector3((float)Math.sin(i/5f), (float)Math.cos(i/5f), 0);
...

		{
			...
			Shaders.OUTPUT.use();
			GL20.glUniform1i(Shaders.OUTPUT.uniform("sourceWidth"), buffers[0].width);
			Shaders.noProgram();
		}

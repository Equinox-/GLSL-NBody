...
public class SpiralGalaxy implements PlacementScript {
	...
	private static void generateGalaxy(float[] masses, Vector3[] pos,
		...
		pos[offset] = new Vector3(0, 0, 0);
		vel[offset] = new Vector3();
		masses[offset] = blackHoleMass;
		...
		for (int i = offset + 1; i < offset + count; ++i) {
			...
			pos[i] = new Vector3(magnitude * (float) Math.cos(angle), magnitude
					* (float) Math.sin(angle), galacticBuldge * lpitch
					+ (diskRadius * randomGauss(-1.0f, 1.0f)));
